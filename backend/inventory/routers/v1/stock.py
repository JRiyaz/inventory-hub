...
from inventory.models.domain import StockLevel, StockMovement, Product, Warehouse
from inventory.schemas.stock import StockAdjustment, StockLevelResponse, StockMovementResponse
from inventory.utils.dependencies import RoleChecker, AuthenticatedUser, get_current_user
...
async def adjust_stock_level(
    ...
    if not product_check.scalar_one_or_none():
        raise HTTPException(
            ...
            detail="Target product not found in catalog"
        )
    ...
    if not stock:
        ...
        if payload.quantity_changed < 0:
            raise HTTPException(
                ...
                detail="Negative stock levels are prohibited. Available: 0"
            )
        stock = StockLevel(
